package com.ankita.driver;
import java.util.Scanner;
import com.ankita.model.Employee;
import com.ankita.service.CredentialService;
public class Driver {
	
	public static void main(String[] args) {
		Employee employee = new  Employee ("Ankita","Tembhurne");
		CredentialService cs = new  CredentialService();
		String generatedEmail;
		char[] generatedPassword;
		
		System.out.println("Please enter the department  from the following");
		System.out.println("1. Technical");
		System.out.println("2. Admin");
		System.out.println("3. Human Resource");
		System.out.println("4. Legal");
		
		Scanner sc = new Scanner(System.in);
		int option = sc.nextInt();
		switch (option) {
		case 1:
			generatedEmail = cs.generatedEmailAddress(employee.getFirsrName().toLowerCase(),
					employee.getLastName().toLowerCase(), "tech");
			generatedPassword = cs.generatedPassword();
			cs.showCredentials(employee, generatedEmail, generatedPassword );
			
		break;
		case 2:
			generatedEmail = cs.generatedEmailAddress(employee.getFirsrName().toLowerCase(),
					employee.getLastName().toLowerCase(), "adm");
			generatedPassword = cs.generatedPassword();
			cs.showCredentials(employee, generatedEmail, generatedPassword );
			
		break;
		case 3:
			generatedEmail = cs.generatedEmailAddress(employee.getFirsrName().toLowerCase(),
					employee.getLastName().toLowerCase(), "hr");
			generatedPassword = cs.generatedPassword();
			cs.showCredentials(employee, generatedEmail, generatedPassword );
		break;
		case 4:
			generatedEmail = cs.generatedEmailAddress(employee.getFirsrName().toLowerCase(),
					employee.getLastName().toLowerCase(), "lg");
			generatedPassword = cs.generatedPassword();
			cs.showCredentials(employee, generatedEmail, generatedPassword );
		break;
		
			
		}
	}
}
